...
$(function(){
	$("button[role='tab']").on('click', function(){
		...
		if(tab === "1") { // 주문관리
			...
			location.href = "${pageContext.request.contextPath}/admin/order/canceled";
		}
	});
});
</script>
	...
	<div class="body-container">	
		...
		<div class="body-main">
			...
			<ul class="nav nav-tabs mt-5" id="myTab" role="tablist">
				...
				<li class="nav-item" role="presentation">
					<button class="nav-link ${orderStatus=='canceled'?'active':''}" id="tab-3" data-bs-toggle="tab" data-bs-target="#tab-pane" type="button" role="tab" aria-controls="3" aria-selected="${orderStatus=='canceled'?'true':'false'}">취소현황</button>
				</li>
			</ul>
			...
			<div class="tab-content pt-4" id="myTabContent">
		        ...
		        <div class="row board-list-header">
					<div class="col-auto me-auto">
		        		<c:if test="${orderStatus =='status'}">
			            	...
			            	<div class="form-check form-check-inline">
			            		<input type="radio" id="order-state0" class="form-check-input" name="orderstatus" value="0" ${state==0 ? "checked='checked'" : ""}> <label class="form-check-label" for="order-state0">전체 주문</label>
			            	</div>
		            	</c:if>
				...
				<table class="table board-list">
					<thead class="table-light">
						<tr>
							<th>주문번호</th>
							<th width="110">주문구분</th>
							<th width="100">주문자</th>
							<th width="150">주문일자</th>
							<th width="130">결제금액</th>
							<th width="150">${orderStatus=="status"?"주문수량":"송장번호"}</th>
							<th width="90">취소건수</th>
							<th width="90">변경</th>
						</tr>
					</thead>
					...
					<tbody>
						<c:forEach var="dto" items="${list}" varStatus="status">
							<tr valign="middle">
								...
								<td>${orderStatus=="status"?dto.totalQuantity:dto.invoiceNumber}</td>
